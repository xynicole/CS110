...
def status_validated(status):
  return status.isalpha() and status == ("single") or status == ('married')

def income_validated(total_income_str):
  return total_income_str.isdigit() and int(total_income_str) > 0


def main():
  status = input('Are you filing single or married? OR <ENTER> to quit   ')
  while status:
    while not status_validated(status):
      print('Invalid Input: input only single or married')
      status = input('Are you filing single or married? OR <ENTER> to quit  ')
      
    total_income_str = input('Please enter your total income:   ')
    while not income_validated(total_income_str):
